...
local oldGui = player:WaitForChild("PlayerGui"):FindFirstChild("FlyingPlatformGUI")
if oldGui then
	oldGui:Destroy()
end
...
RunService.Heartbeat:Connect(function(dt)
	...
	-- Posición anterior de la plataforma
	local oldPlatformY = platform.Position.Y
	...
	-- SEGUIR LOS PIES
		...
		targetY = root.Position.Y - FOOT_OFFSET
	end

	-- Movimiento horizontal siempre sigue al jugador
	...
	-- Si está ascendiendo o descendiendo,
	-- mueve al personaje junto con la plataforma.
	if ascending or descending then

		local deltaY = targetY - oldPlatformY

		if math.abs(deltaY) > 0 then
			root.CFrame = root.CFrame + Vector3.new(0, deltaY, 0)
		end
	end
end)
